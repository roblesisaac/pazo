function addMethodToArray(name, fn) {
                ...
                if (Array.isArray(val)) {
                    ...
                    for (var i = 0; i < val.length; i++) {
                        ...
                        if (val[i] !== undefined) {
                            ...
                            typeof item == "object" ? Object.loop(item, fn, parent) : fn(val, i, item, parent);
    ...
    function CSVToArrays(stringData, strDelimiter) {
        ...
        var objPattern = new RegExp(( // Delimiters.  "(\\" + strDelimiter + "|\\r?\\n|\\r|^)" +  // Quoted fields.  "(?:\"([^\"]*(?:\"\"[^\"]*)*)\"|" +  // Standard fields.  "([^\"\\" + strDelimiter + "\\r\\n]*))"), "gi");  var arrData = [[]];  var arrMatches = null;  while (arrMatches = objPattern.exec(stringData)) {    var strMatchedDelimiter = arrMatches[1];    if (strMatchedDelimiter.length && (strMatchedDelimiter != strDelimiter)) {      arrData.push([]);    }    if (arrMatches[2]) {      var strMatchedValue = arrMatches[2].replace(      new RegExp("\"\"", "g"), "\"");    } else {      var strMatchedValue = arrMatches[3];    }    arrData[arrData.length - 1].push(strMatchedValue);  }  return (arrData);}function convertCsvToJson(csv, delimeter) {  var array = CSVToArrays(csv, delimeter);  var objArray = [];  for (var i = 1; i < array.length; i++) {    objArray[i - 1] = {};    for (var k = 0; k < array[0].length && k < array[i].length; k++) {      var key = array[0][k];      objArray[i - 1][key] = array[i][k]    }  }  return objArray;}if (!String.prototype.includes) {  String.prototype.includes = function() {    "use strict";    return String.prototype.indexOf.apply(this, arguments) !== -1;  };}if (!String.prototype.excludes) {  String.prototype.excludes = function() {    "use strict";    return String.prototype.indexOf.apply(this, arguments) === -1;  };}if (typeof String.prototype.parseFunction != "function") {  String.prototype.parseFunction = function () {    var funcReg = /function *\(([^()]*)\)[ \n\t]*{(.*)}/gmi;    var match = funcReg.exec(this.replace(/\n/g, " "));    if(match) {      return new Function(match[1].split(","), match[2]);    }    return null;  };}
